...
class Migration_Create_Duty extends CI_Migration {
    ...
    public function up()
    {
        ...
        $this->db->query("
            ...
            CONSTRAINT duty_fk_wiw_master FOREIGN KEY ({$prefix}_master_id) REFERENCES {$this->db->dbprefix("who_is_where")} (wiw_id) ON DELETE CASCADE ON UPDATE CASCADE ,
            CONSTRAINT duty_fk_wiw_employee FOREIGN KEY ({$prefix}_employee_id) REFERENCES {$this->db->dbprefix("who_is_where")} (wiw_id) ON DELETE CASCADE ON UPDATE CASCADE ,
