...
class Migration_Create_Parent extends CI_Migration {
    ...
    public function up()
    {
        ...
        $this->db->query("
            ...
            CONSTRAINT parent_fk_user_master FOREIGN KEY ({$prefix}_master_id) REFERENCES {$this->db->dbprefix("user")} (usr_id) ON DELETE CASCADE ON UPDATE CASCADE ,
            CONSTRAINT who_is_where_fk_user_employee FOREIGN KEY ({$prefix}_employee_id) REFERENCES {$this->db->dbprefix("user")} (usr_id) ON DELETE CASCADE ON UPDATE CASCADE ,
